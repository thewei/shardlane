...
Herdr 0.8.2 renders Sidebar/tab chrome on the server. A native A/B showed that letting the server render chrome and cropping it in Shardlane cost about **1.2–1.8 ms/frame** in the projection/cloning path; when a fresh Shardlane-started server and child both use the merged runtime config with Sidebar/tab chrome removed at source, the same projection seam was effectively **0–1 µs** and `projected=false`. Archived evidence only: the 2026-09-18 fixed-Tab-presentation ruling (Sidebar Tab subtree + the hosted TUI's own always-visible chrome) deleted the crop seam entirely — `TuiChromeProjection`, `TerminalFrame::project_rect`, the chrome probes, and the compensated-grid resize path no longer exist. The visible grid IS the raw TUI grid; `stage=frame.apply` traces no longer carry `projected`/`projection_us` fields.
