...
**Current (2026-09-19 revision) — Sidebar/breadcrumb Tab actions, Herdr-config Tab bar hiding.** Tab placement is not configurable. The canonical Sidebar renders the per-Project Tab subtree plus an explicit "新增 Tab" row (creation through `tab.create`), with Rename/Close/drag reorder on the Tab rows (`tab.move`/`tab.rename`/`tab.close`; `tab.moved` stays the correction channel); the Header breadcrumb's Tab layer adds a hover-revealed "…" menu whose Rename/Close items route through the same `tab.rename`/`tab.close` channels — no second action implementation exists. The hosted Herdr TUI hides its own Tab bar while a window shows a single Tab: Shardlane normalizes Herdr `ui.hide_tab_bar_when_single_tab` to `true` at startup (one-shot `migrations.tui_tab_bar_hidden` flag; the key is not exposed in Settings). Herdr 0.9.1's only visibility rule is `show_tab_bar = rows > 1 && !(hide && tab_count == 1)` — there is no always-hide key, so with two or more Tabs Herdr still draws its own Tab bar. That is a documented Herdr boundary gap: the sanctioned fix is an upstream Herdr key (e.g. `ui.hide_tab_bar`), never a client-side chrome crop. The former `terminal.tab_bar_placement` selector (`sidebar`/`native`/`herdr-tui`), the native content-area Tab strip (`shell_tabs.rs`), and the client-side TUI chrome-crop projection remain deleted: the visible Ghostty grid IS the raw TUI grid, so terminal geometry, mouse hit-testing, selection mapping, and the shared-session resize/adopt paths carry no chrome compensation. Herdr's Tab order and lifecycle stay authoritative; the Sidebar tree and breadcrumb menu are presentation-only and never a second runtime or navigation implementation.
