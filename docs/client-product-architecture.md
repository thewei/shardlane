...
**Current (2026-09-18) — fixed Tab presentation.** Tab placement is no longer configurable. The canonical Sidebar always renders the per-Project Tab subtree (Pin/Rename/Close/drag reorder through the existing Sidebar Tab action paths: `tab.create`/`tab.move`/`tab.rename`/`tab.close`; `tab.moved` stays the correction channel), and the hosted Herdr TUI always keeps its own chrome — its Tab bar included, even with a single Tab (Shardlane normalizes Herdr `ui.hide_tab_bar_when_single_tab` to `false` at startup and no longer exposes the key in Settings). The former `terminal.tab_bar_placement` selector (`sidebar`/`native`/`herdr-tui`), the native content-area Tab strip (`shell_tabs.rs`), and the client-side TUI chrome-crop projection were deleted: the visible Ghostty grid IS the raw TUI grid, so terminal geometry, mouse hit-testing, selection mapping, and the shared-session resize/adopt paths carry no chrome compensation. Herdr's Tab order and lifecycle stay authoritative; the Sidebar tree is presentation-only and never a second runtime or navigation implementation.
