...
- Tab presentation (revised 2026-09-19): the canonical Sidebar renders the per-Project Tab subtree plus an explicit "新增 Tab" row (creation through Herdr `tab.create` via `create_tab_in_workspace`), and the Header breadcrumb's Tab layer carries a hover-revealed "…" menu whose Rename/Close items call the same `open_tab_rename`/`close_tab_by_id` channels as the Sidebar rows — no second action implementation. The hosted Herdr TUI hides its own Tab bar while the window has a single Tab — startup normalizes Herdr `ui.hide_tab_bar_when_single_tab` to `true` (one-shot `migrations.tui_tab_bar_hidden`; Settings does not expose the key). Herdr 0.9.1 has no always-hide key (`show_tab_bar = rows > 1 && !(hide && tab_count == 1)`): with ≥2 Tabs Herdr still draws its own Tab bar — a documented upstream gap to solve at the Herdr boundary, never by cropping. The `terminal.tab_bar_placement` setting, the native content-area Tab strip (`shell_tabs.rs`), and the client-side TUI chrome-crop projection (`TuiChromeProjection`, chrome probes, compensated-grid resize) remain deleted; do not reintroduce a second Tab presentation owner or crop the TUI's chrome — the visible Ghostty grid IS the raw TUI grid.
