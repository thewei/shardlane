...
- Tab presentation is fixed (2026-09-18): the canonical Sidebar always renders the per-Project Tab subtree, and the hosted Herdr TUI always keeps its own chrome — Tab bar included, even with a single Tab (startup normalizes Herdr `ui.hide_tab_bar_when_single_tab` to `false`; Settings no longer exposes it). The `terminal.tab_bar_placement` setting, the native content-area Tab strip (`shell_tabs.rs`), and the client-side TUI chrome-crop projection (`TuiChromeProjection`, chrome probes, compensated-grid resize) were deleted; do not reintroduce a second Tab presentation owner or crop the TUI's chrome.
