...
- `src/adapters/` — external Agent format boundary; readers only. Supported formats: claude, codex, copilot, cursor, opencode, command-code, kiro, gemini, pi (omp shares the pi parser with the `~/.omp` root), grok, kimi, antigravity (SQLite summaries store; metadata cards only — live turn bodies are read by `antigravity_live.rs`, which walks the plaintext protobuf paths in `conversations/<uuid>.db` steps: `.5.1` timestamp + `.20.1` assistant reply, fail-closed on malformed payloads; missing timestamps degrade to 0), dsh (zstd multi-frame event log), qoder (active-leaf JSONL under `~/.qoder/projects`). Shared parsing helpers live in `parse_utils.rs` (`blocks_text`/`MtimeCache` included); read-only SQLite access lives in `sqlite_ro.rs`. The codex adapter additionally decodes pending approval requests from rollout event_msg rows (`exec_approval_request` / `apply_patch_approval_request`) into `LiveApproval` facts, with replayable option keys sourced only from the verified Codex TUI keymap contract (vendor defaults plus CODEX_HOME config.toml overrides) — adapters that cannot derive options/keys stably must not emit approvals (never-blind-send).
- `src/sources.rs` — `ApplicationConfig`-owned `HistorySourcePolicy`, enabled/disabled default and custom location snapshots, and one adapter-roster generation shared by scanner/watch/detail/export/live routing; it never writes the disposable catalog.
 - `src/catalog.rs` — Shardlane-owned SQLite/FTS catalog plus disposable page-addressable transcript cache (`transcript_page_meta` / `transcript_page_cache` / `transcript_message_index`) and CJK-aware short-query policy; never an external Agent database. Message-FTS is a bounded index (retention window over non-archived/most-recent sessions, v4 migration prune → 'optimize' → VACUUM, INCREMENTAL auto_vacuum) so the catalog can never re-inflate toward source size.
 - `src/scanner.rs` — parse/index orchestration; mtime-incremental with missing-source reconciliation, changed-source transcript page-cache prewarming from the same adapter parse pass, and post-scan message-FTS prune plus a bounded per-round merge budget.
