        ...
        div()
            ...
            .child(
                ...
                div()
                    ...
                    .when(selected_section == SettingsSection::Machines, |page| {
                        page.child(self.machines_settings_content(window, cx))
                    })
...
impl ShardlaneApp {
    /// Machines settings page (B1): the local machine plus SSH targets. Each
    /// SSH machine brings up a unix-socket bridge (`ssh_bridge::bring_up`) so
    /// its Herdr instances become bindable from the workspace switcher.
    pub(crate) fn machines_settings_content(
        &mut self,
        window: &mut Window,
        cx: &mut Context<Self>,
    ) -> impl IntoElement {
        ...
        if self.machine_ssh_input.is_none() {
            self.machine_ssh_input = Some(
                cx.new(|cx| InputState::new(window, cx).placeholder("user@host or SSH alias…")),
            );
        }
        let ssh_control = self
            .machine_ssh_input
            .as_ref()
            .map(|input| {
                Input::new(input)
        ...
        let mut device_rows = v_flex().gap(px(2.0));
        for device in &devices {
            let row = match device.ssh_target.clone() {
                Some(target) => {
                    let bridged = self
                        .shared
                        .ssh_bridges
                    ...
                    h_flex()
                        ...
                        .child(
                            div()
                                ...
                                .child(if bridged { "bridged" } else { "SSH · down" }),
                        )
                }
                None => h_flex()
                    .w_full()
                    ...
                    .child(
                        div()
                            .text_size(px(12.0))
                            .text_color(component_theme.foreground)
                            .child(format!("{} (local)", local_name)),
                    )
                    .child(
                        div()
                            .text_size(px(11.0))
                            .text_color(component_theme.muted_foreground)
                            .child(format!("herdr {}", local_version)),
                    ),
            };
            device_rows = device_rows.child(row.into_any_element());
        }
        ...
        v_flex()
            ...
            .child(
                div()
                    ...
                    .text_color(component_theme.muted_foreground)
                    .child(
                        "Connect this Mac's Herdr daemon, or reach another machine over SSH.",
                    ),
            )
            .child(device_rows)
            .child(
