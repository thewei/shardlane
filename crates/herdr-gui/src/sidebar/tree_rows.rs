...
//! [OUTPUT]: Provides ShardlaneApp's history rows, workspace agent status aggregation, and project/tab row rendering (including drag initiation and drop targets: tabs map through the authoritative order, projects reorder via Herdr, load more). Project rows project a git identity trailing (branch + working-tree +/- counts) plus a running-services count pill (hover-revealed Services entry when zero; both open the Services panel) from the shared scripts counter; Tab leads use the square-terminal glyph with a success service badge when an observed service runs inside the Tab.
