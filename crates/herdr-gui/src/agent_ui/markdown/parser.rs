...
#[allow(clippy::expect_used)] // static, compile-time-constant regex pattern
...
#[allow(clippy::enum_variant_names)] // Original names BlockQuote/BlockTree are semantically clear; renaming would be pointless churn
#[derive(Clone, Debug, PartialEq)]
pub enum Block {
    Paragraph {
        runs: Vec<InlineRun>,
...
mod tests {
    ...
    fn explicit_links_and_inline_code_are_not_relinkified() {
        let tree = parse(
            "[docs at https://example.com](https://example.org) and `https://example.com/code`",
        );
        let Block::Paragraph { runs } = &tree.blocks[0].block else {
            panic!("expected a paragraph");
        };
        ...
        assert!(runs.iter().any(|run| {
            run.text == "docs at https://example.com"
                && run.style.link.as_deref() == Some("https://example.org")
        }));
        assert!(runs.iter().any(|run| {
