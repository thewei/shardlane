// Intentionally relicensed for Shardlane. This file deliberately carries no
// repository-level GPL declaration; third-party material (pulldown-cmark,
// regex, Lucide iconography) retains its upstream notices in
// THIRD_PARTY_NOTICES.md. Shardlane adaptations: palette resolved from a caller
// -supplied PaletteSource (Shardlane theme tokens) instead of an internal theme,
//! tooltips via Shardlane ui::tooltip, icons via Shardlane Icon assets.
...
use gpui::{
    canvas, div, font, img, point, prelude::*, px, quad, relative, size, AnyElement, BorderStyle,
    Bounds, ClipboardItem, CursorStyle, DispatchPhase, Font, FontStyle, FontWeight, Hsla,
    InteractiveText, IntoElement, KeyDownEvent, MouseButton, MouseDownEvent, MouseMoveEvent,
    MouseUpEvent, ParentElement, Pixels, Point, SharedString, StrikethroughStyle, StyledText,
...
use super::selection::{
    line_range, word_range, RegisteredText, SelectionRegistry, SelectionState, TextKey,
};
use super::veil::{apply_veil, RowVeil};
...
/// Shardlane asset icon (replaces a shared ui::icon helper).
fn md_icon(path: &'static str, size: f32, color: Hsla) -> AnyElement {
...
impl Palette {
    /// Shardlane-side palette input (derived by the caller from theme tokens;
    /// see `palette_source_from_active` in mod.rs).
    pub fn from_source(source: PaletteSource) -> Self {
        let search_yellow = gpui::hsla(
            48.0 / 360.0,
            0.95,
            if source.is_dark { 0.48 } else { 0.55 },
            1.0,
        );
        let active_search_orange = gpui::hsla(
            30.0 / 360.0,
            1.0,
            if source.is_dark { 0.50 } else { 0.54 },
            1.0,
        );
        Self {
            text: source.text,
            ...
            code_text: source.code_text,
            code_wash: source.code_wash,
            selection: source.selection,
            search_match: search_yellow.opacity(if source.is_dark { 0.18 } else { 0.20 }),
            active_search_match: active_search_orange.opacity(if source.is_dark {
                0.78
            } else {
                0.70
            }),
            accent: source.accent,
            added: source.success,
            removed: source.danger,
            is_dark: source.is_dark,
        }
    }
}

/// Explicit input surface from Shardlane theme tokens to the Markdown palette.
#[derive(Clone, Copy, Debug, PartialEq)]
...
#[allow(clippy::too_many_arguments)] // Original signature: every argument is an explicit color/switch; struct-ifying would be pointless churn
...
/// It is also the installation point for selection input: `install_selection_input`
/// relies on window mouse handlers registered during paint — previously never
/// called, drag selection was completely broken on both surfaces
/// (notate 08-29, five rounds). Handlers re-register every frame; idempotent.
pub fn frame_reset(selection: TranscriptSelection) -> impl IntoElement {
    canvas(
        |_, _, _| (),
        move |_, _, window, _| {
            install_selection_input(window, &selection);
...
fn range_rects(
    ...
    for line in text_layout_lines(layout) {
...
// gpui 0.2.2's TextLayout has no safe "shaped" probe (len()/bounds() panic
// before measurement), while range_rects is only called during the same
// frame's paint phase — by then sibling StyledText has necessarily completed
// measurement. That invariant replaces the original layout_missing defense.
fn layout_missing(_layout: &TextLayout) -> bool {
    false
}

/// Enumerate shaped line layouts line by line (gpui 0.2.2 has no line_layouts()
/// iterator; the line-advance step matches TextLayout's internal paint loop:
/// line.len() + 1).
fn text_layout_lines(layout: &TextLayout) -> Vec<std::sync::Arc<gpui::WrappedLineLayout>> {
...
fn show_code_copied(feedback: CodeCopyFeedback, ordinal: usize, cx: &mut gpui::App) {
    ...
    cx.spawn(async move |cx| {
        ...
        if clear_code_copy_feedback(&feedback, ordinal, generation) {
            // A failed refresh from an async callback only means the view is
            // gone; nothing to propagate.
            let _ = cx.refresh();
        }
    })
    .detach();
...
fn render_code_block(language: Option<&str>, code: &str, ctx: &Ctx) -> AnyElement {
    ...
    let copy_button = div()
        ...
        .tooltip(crate::ui::tooltip::tooltip_fn(if copied {
            "Copied"
        } else {
            "Copy code"
        }))
...
mod tests {
    use super::*;
    use crate::agent_ui::markdown::parser;

    fn light_palette() -> Palette {
        Palette::from_source(PaletteSource {
            text: gpui::black(),
            secondary: gpui::black().opacity(0.7),
            tertiary: gpui::black().opacity(0.55),
            ghost: gpui::black().opacity(0.4),
            border: gpui::black().opacity(0.1),
            inset: gpui::black().opacity(0.04),
            overlay: gpui::black().opacity(0.07),
            code_text: gpui::black(),
            code_wash: gpui::black().opacity(0.035),
    ...
    fn palette() -> Palette {
        Palette::from_source(PaletteSource {
            text: gpui::white(),
            secondary: gpui::white().opacity(0.7),
            tertiary: gpui::white().opacity(0.55),
            ghost: gpui::white().opacity(0.4),
            border: gpui::white().opacity(0.1),
            inset: gpui::white().opacity(0.05),
            overlay: gpui::white().opacity(0.08),
            code_text: gpui::white(),
            code_wash: gpui::white().opacity(0.04),
            selection: gpui::blue().opacity(0.3),
            accent: gpui::blue(),
            success: gpui::green(),
            danger: gpui::red(),
    ...
    fn a_style_change_drops_cached_flats() {
        ...
        let cached = view.flat(0, || {
            flatten(&runs_of("a"), &dark, FontWeight::NORMAL, dark.text)
        });
        ...
        let relit = view.flat(0, || {
            flatten(&runs_of("a"), &light, FontWeight::NORMAL, light.text)
        });
