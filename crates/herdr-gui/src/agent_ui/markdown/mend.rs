...
pub const PENDING_LINK_URL: &str = "shardlane:pending-link";
...
pub fn close_hanging(text: &str) -> Option<String> {
    ...
    let mut closers = String::new();
    let open_code =
        code.filter(|&(_, content_at)| last_content.is_some_and(|content| content >= content_at));
    if let Some((run, _)) = open_code {
        closers.extend(std::iter::repeat_n('`', run));
    }
    close_delimiters(&mut closers, &delimiters, last_content, chars.len());
    let pending_bracket = brackets
        .last()
        .copied()
        .filter(|&bracket| last_content.is_some_and(|content| content > bracket));
