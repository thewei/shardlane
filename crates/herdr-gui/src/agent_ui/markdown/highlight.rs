...
pub fn tokenize_line(lang: Lang, line: &str, carry: Carry) -> (Vec<Token>, Carry) {
    ...
    while index < bytes.len() {
        ...
        let block_delimiters = spec
            .block_comment
            .and_then(|(open, close)| rest.starts_with(open).then_some((open, close)));
...
fn markdown_line(line: &str, carry: Carry) -> (Vec<Token>, Carry) {
    ...
    let reference_target = if content.starts_with('[') {
        find_unescaped(line, content_start + 1, b']')
            .filter(|&close| line.as_bytes().get(close + 1) == Some(&b':'))
    } else {
        None
    };
    if let Some(close) = reference_target {
        ...
        match bytes[index] {
            ...
            _ => {
                match rest.chars().next() {
                    ...
                    // Unreachable (this arm only fires on a multi-byte lead
                    // byte); advance anyway to rule out an infinite loop.
                    None => index += 1,
                }
            }
        }
