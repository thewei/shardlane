...
impl ShardlaneApp {
    ...
    pub(super) fn set_terminal_frame(
        ...
        // round AND was last round (equal, empty projections): a chrome projection crops
        // and re-indexes rows, so raw row indices would not describe the painted grid.
        // Anything else keeps the plan out of the pane path (correctness over speed).
        ...
        if raw_unchanged && self.terminal_frame_projection == projection {
            if trace {
                crate::terminal_trace::event(format_args!(
                    "stage=frame.apply unchanged=true planned={} projected={} rows={} compare_us=0 total_us={}",
                    applied_with_plan,
                    !projection.is_empty(),
        ...
        if unchanged {
            ...
            if trace {
                crate::terminal_trace::event(format_args!(
                    "stage=frame.apply unchanged=true projected={projected} rows={} compare_us={compare_us} total_us={}",
                    frame.lines.len(),
                    apply_started
                        .map(crate::terminal_trace::elapsed_us)
                        .unwrap_or(0),
        ...
        if trace {
            crate::terminal_trace::event(format_args!(
                "stage=frame.apply unchanged=false projected={projected} rows={} compare_us={compare_us} pane_update_us={} total_us={}",
                self.terminal_frame.lines.len(),
                pane_started
                    .map(crate::terminal_trace::elapsed_us)
                    .unwrap_or(0),
